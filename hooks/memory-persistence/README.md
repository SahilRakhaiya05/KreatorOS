...
These lifecycle hook definitions document the KreatorOS memory persistence contract for Claude Code plugin and manual installs.
...
- `session-activity-tracker.js` records tool usage and file activity for KreatorOS status and observability.
...
| `PostToolUse` | `post:session-activity-tracker` | Record tool and file activity for KreatorOS metrics | no |
...
- Bound context loaded at session start with `KREATOROS_SESSION_START_MAX_CHARS`.
- Allow opt-out with `KREATOROS_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `KREATOROS_HOOK_PROFILE` and `KREATOROS_DISABLED_HOOKS`.
